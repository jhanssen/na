...
class GMusicWriteStream extends Writable
{
    constructor(options) {
        super(options);
...
class GMusicReadStream extends Readable
{
    constructor(options) {
        super(options);

        this._write = new GMusicWriteStream({ readStream: this });
        ...
        request
            .get(this._url)
            .on("error", err => {
                console.error("gmusic read stream error", err);
            })
            .on("end", () => {
                this._done = true;
                while (!this._write._chunks.length)
    ...
    stream() {
        return new Promise((resolve, reject) => {
            pm.getStreamUrl(this._item.storeId, (err, url) => {
                ...
                resolve(new GMusicReadStream({ url: url }));
            });
        });
    }
