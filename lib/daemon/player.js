...
const player = {
    ...
    _input: require(appRoot + "/lib/daemon/inputs/file"),
    _gain: 1,
    ...
    handle: function(data) {
        return new Promise((resolve, reject) => {
            ...
            case "set":
                this._set(args(data.argv)).then(resolve).catch(reject);
                break;
            case "clear":
            ...
            case "list": {
                ...
                for (let i = 0; i < playlist.length; ++i) {
                    const meta = this._playlist.getMetadata(playlist[i]);
            ...
            default:
                if (this._input) {
                    this._input.handle(data.cwd, cmd(data.argv), args(data.argv)).then(resolve).catch(reject);
                } else {
    ...
    setup: function() {
        this._input.setPlaylist(this._playlist);
    ...
    _play: function() {
        return new Promise((resolve, reject) => {
            ...
            if (typeof file.stream === "function") {
                this._playing.stream = file.stream();
                this._playing.decoder = decoderFor(file.ext());
