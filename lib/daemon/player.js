...
function actualVersion(ver)
{
    // if we have more than two dots, remove the extra
    let prev = 0, cnt = 0;
    for (;;) {
        const idx = ver.indexOf(".", prev);
        if (idx == -1)
            break;
        if (++cnt == 3) {
            // kill everything past this point
            return ver.substr(0, idx);
        }
        prev = idx + 1;
    }
    return ver;
...
const isITerm = (process.env.TERM_PROGRAM == "iTerm.app" && semver.gte(actualVersion(process.env.TERM_PROGRAM_VERSION), "3.0.0"));
...
const queue = {
    _playing: { stream: undefined, decoder: undefined, multiplier: undefined, speaker: undefined },
    _stopping: false,
    _volume: 1,
        ...
        return new Promise((resolve, reject) => {
            ...
            if (file && typeof file.stream === "function") {
                file.stream().then(stream => {
                    ...
                    this._playing.multiplier = new Multiply({ gain: this._volume });
    ...
    handle: function(data) {
        return new Promise((resolve, reject) => {
            ...
            case "load":
                ...
                break;
            case "volume": {
                const v = firstArg(data.argv._);
                if (typeof v === "number" && v >= 0 && v <= 1) {
                    queue.setVolume(v);
                    resolve();
                    return;
                }
                reject(`Invalid volume ${v}`);
                break; }
            case "list": {
